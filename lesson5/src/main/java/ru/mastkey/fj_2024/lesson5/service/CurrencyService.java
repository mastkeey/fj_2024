...
public class CurrencyService {
    ...
    public CurrencyRateResponse getCurrencyRate(String code) {
        ...
        return conversionService.convert(currencies.stream()
                .filter(cbCurrencyResponse -> Objects.equals(cbCurrencyResponse.getCharCode(), code))
                .findFirst()
                .orElseThrow(() -> new ServiceException(ErrorType.NOT_FOUND, MSG_CURRENCY_NOT_SUPPORTED, code)),
    ...
    private void validateCurrencyCode(String code) {
        try {
            var currencyTo = Currency.getInstance(code);
